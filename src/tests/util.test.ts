...
describe('emoji util test', () => {
  it('creates search tokens', () => {
    expect(compileSearchTokens('joy   "cat " ^grin|at$')).toMatchObject([
      'joy',
      'cat ',
      '^grin',
      'at$',
  ...
  it('finds search token matches', () => {
    const s = 'the cat'
    expect(findTokenMatches(s, compileSearchTokens('^th'))).toMatchObject([
      [0, 1],
    ])
    ...
    expect(findTokenMatches(s, compileSearchTokens('!at$'))).toMatchObject([])
    expect(findTokenMatches(s, compileSearchTokens('cat'))).toMatchObject([
      [4, 6],
    ])
    expect(findTokenMatches(s, compileSearchTokens('"e c"'))).toMatchObject([
      [2, 4],
    ])
    expect(findTokenMatches(s, compileSearchTokens('e c'))).toMatchObject([
      [2, 2],
    ...
    expect(findTokenMatches(s, compileSearchTokens('^th th'))).toMatchObject([
      [0, 1],
      [0, 1],
    ])
    expect(
      findTokenMatches('ON! arrow', compileSearchTokens('!'))
    ).toMatchObject([[2, 2]])
    expect(
      findTokenMatches('ON! arrow', compileSearchTokens('"! a"'))
    ).toMatchObject([[2, 4]])
  })
})
