...
const migrations: Migration[] = [
  {
    ...
    run: (store: AbstractStorageService): void => {
      ...
      for (const [oldKey, newKey] of Object.entries(map)) {
        const value = dotGet(currentState, oldKey)
        if (value != null) {
...
export function runMigrations(
  store: AbstractStorageService,
  toVersion?: string
): Promise<void> {
  const storedVersion = store.get('appVersion', '1.0.0') as string
  toVersion ??= APP_VERSION
  if (storedVersion === toVersion) return Promise.resolve()

  if (!store.isEmpty()) {
    let index = migrations.findIndex((m) =>
      isSemverGreater(m.version, storedVersion)
    )
    if (index !== -1) {
