...
const keyedWritable = <Type>(
  ...
  const sync = (v: Type): void => {
    void storageService.set(key, v).save().then()
  }
...
export const getSyncedStore = <Type>(
  key: keyof AppState,
  defaultValue: Type
): Writable<Type> => {
  if (!Object.hasOwn(storesDirectory, key)) {
    storesDirectory[key] = keyedWritable(
      key,
      storageService.get(key) ?? defaultValue
    )
  }

  return storesDirectory[key] as Writable<Type>
}
