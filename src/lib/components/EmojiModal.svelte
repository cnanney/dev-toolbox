  ...
  function copyImg() {
    fetch(emojiPngPath(emoji)).then((response) => {
      navigator.clipboard.write([
        new ClipboardItem({ 'image/png': response.blob() }),
      ])
    })
  }
...
<div
  class="absolute bg-white dark:bg-gray-700 inset-0 align-middle px-4"
...
>
  ...
  >
    ...
    <div class="p-4 pb-6 flex flex-col text-center justify-between h-full">
      ...
      <div
        class="grid grid-cols-2 text-gray-800 dark:text-gray-200 content-center"
      >
        <img
          class="w-max mx-auto"
          src={emojiPngPath(emoji)}
          title="Google PNG"
          alt="Google PNG"
        />
        <div>
          <span class="text-6xl" title="Your Browser"
            >{@html emojiHtmlChars(emoji)}</span
          >
        </div>
      ...
      <div>
        <h3
          class="mb-2 text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase"
        >
          Codepoint{emoji.qc.length !== 1 ? 's' : ''}
        </h3>
        <p
          class="font-mono text-xs flex dark:text-gray-200 space-x-3 justify-center"
        >
      ...
      {#if emoji.gh.length}
        <div>
          ...
          <p class="font-mono text-xs dark:text-gray-200">
            {emojiGhCodes(emoji)}
          </p>
        </div>
      {/if}
      ...
      <div>
        <h3
          class="mb-2.5 text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase"
        >
          Copy As
        </h3>
