...
function toBase(
  ...
  try {
    const string = BigInt(input).toString(base)
...
export const time = {
  unix_sec: (input: TimeInput) => String(Math.floor(input?.toSeconds() || 0)),
  unix_milli: (input: TimeInput) => String(input?.toMillis()),
  iso: (input: TimeInput) => input?.toISO({ suppressMilliseconds: true }) || '',
  iso_date: (input: TimeInput) => input?.toISODate() || '',
  iso_time: (input: TimeInput) =>
    input?.toISOTime({ suppressMilliseconds: true }) || '',
  iso_week: (input: TimeInput) => input?.toISOWeekDate() || '',
  sql: (input: TimeInput) =>
    input?.toSQL({ includeOffset: false })?.replace(/\.000$/, '') || '',
  http: (input: TimeInput) => input?.toHTTP() || '',
  local: (input: TimeInput) =>
    input?.toLocal().toISO({ suppressMilliseconds: true }) || '',
  full: (input: TimeInput) =>
    input?.toLocal().toLocaleString(DateTime.DATETIME_FULL_WITH_SECONDS) || '',
}

export const net = {
  ip_dec: (input: NetInput) => String(input?.netLong),
  dec_ip: (input: NetInput) => String(input?.base),
  ip_bin: (input: NetInput) => toBase(String(input?.netLong ?? ''), 2, 32),
  ip_hex: (input: NetInput) => toBase(String(input?.netLong ?? ''), 16),
  net_mask: (input: NetInput) =>
    input?.broadcast ? input.base + '/' + input.mask : '',
  broadcast: (input: NetInput) => input?.broadcast || '',
  hosts: (input: NetInput) => String(input?.broadcast ? input?.size : ''),
