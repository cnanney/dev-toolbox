<script lang="ts">
  ...
  import { DateTime, type DurationLikeObject, type ToSQLOptions } from 'luxon'
  ...
  function adjust(to: string | null = null) {
    ...
    let now: DateTime = DateTime.now().toUTC(),
      base: DateTime =
        to === null
          ? now
          : (preCalc(currentTime, currentTimeType) as DateTime) || now,
    ...
    let val = (
      to === null ? base : base.plus({ [to as keyof DurationLikeObject]: 1 })
    )
      // @ts-ignore
      [method](opts[method] || {})

    timeInput.set(
      typeof val === 'string' ? val.replace(/\.000$/, '') : String(val)
    )
  }
  ...
  const preCalc: PreCalcCallbackWithType = (value, type): TimeInput => {
    try {
      let luxonInput = ['fromMillis', 'fromSeconds'].includes(type)
        ...
        : value
      // @ts-ignore
      let parsed = DateTime[type as TimeTypeFromFormats](luxonInput, {
        zone: 'utc',
      }) as DateTime
      ...
      cols: [
        {
          title: 'Unix (Seconds)',
          size: 1,
          method: time.unix_sec,
          sendInputType: 'fromSeconds',
        },
        {
          title: 'Unix (Milliseconds)',
          size: 1,
          method: time.unix_milli,
          sendInputType: 'fromMillis',
        },
        {
          title: 'ISO 8601',
          size: 1,
          method: time.iso,
          sendInputType: 'fromISO',
        },
      ],
    },
    {
      cols: [
        {
          title: 'ISO Week',
          size: 1,
          method: time.iso_week,
          sendInputType: 'fromISO',
        },
        {
          title: 'ISO Date',
          size: 1,
          method: time.iso_date,
          sendInputType: 'fromISO',
        },
        {
          title: 'ISO Time',
          size: 1,
          method: time.iso_time,
          sendInputType: 'fromISO',
        },
      ],
    },
