...
<svelte:window on:keydown={({key}) => key === 'Escape' && closeEverything()}/>
    ...
    <div class="flex flex-col relative">
        <div class="relative">
                ...
                <HelpText title="Extended search options" position="top" on:close={toggleSearchHelp}>
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td>Items that do not start with <InlineCode>family</InlineCode></td>
                        </tr>
                        <tr>
            ...
            {#if versionHelpVisible}
                <HelpText title="Emoji v15.0 support is limited" position="bottom" classes="mb-3" on:close={toggleHelp}>
                    <p class="text-vs text-gray-700 dark:text-gray-300">If you see one or more "&#9647" (or other
                        garbled characters) when viewing emoji details, that means your OS/browser emoji font does not
                        yet natively support that character.</p>
                </HelpText>
            {/if}
        </div>
        <div class="e-f flex bg-gray-50 dark:bg-gray-800 dark:text-gray-400 py-1.5 px-3 text-xs justify-between shadow-inner">
            <span>Emoji v15.0, Fully-Qualified 
                <button title="Emoji support information" on:click={toggleHelp}><sup>[?]</sup></button>
            </span>
